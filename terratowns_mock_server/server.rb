...
# we will mock having a state or database for thi development server
# by setting a global variable. You would never use a global variable
# in production server.
$home = {}

# This is a ruby class that includes validations form ActiveRecord.
# This will represent our Home resources as a ruby object.
class Home
  # ActiveModel is part of Ruby on Rails.
  # It is used as an ORM. It has a mpdule within
  ...
  # create some virtual atteibutes to stored on this object
  ...
  # we will make sure it an incemental version in the controller
  validates :content_version, numericality: { only_integer: true }
end

# We are extending a class from Sinatra:: Base to
# turn this generic classs to utilize the sintra web-framework.
class TerraTownsMockServer < Sinatra::Base
  ...
  # return the hardcoded access token
  def x_access_code
  ...
  def find_user_by_bearer_token
    ...
    # Check if the Authorizationvheader exists?
    if auth_header.nil? || !auth_header.start_with?("Bearer ")
      error 401, "a1000 Failed to authenicate, bearer token invalid and/or teacherseat_user_uuid invalid"
    end

    # Does the token match the one in our database?
    # if we cat find it that return an errir or if it doesn't match
    ...
    # in rails: User.find_by accesscode: code, user_uuid
  ...
  post '/api/u/:user_uuid/homes' do
    ...
    # putss will print to the terminal similar to a print ort console.log
    puts "# create - POST /api/homes"

    # a begin/resource is a try/catch, if an eerrors occurs, result it
    begin
      # Sinatra does not automatically part json bodys as params
      # like rails so we need to manual parse it
    ...
    # Create a new Home model and set the atteibtes
    home = Home.new
    home.town = town
    home.name = name
    ...
    # nesure our validation checks pass otherwise
    # return the error
    unless home.valid?
      error 422, home.errors.messages.to_json
    end

    # enerating a uuid at random
  ...
  get '/api/u/:user_uuid/homes/:uuid' do
    ...
    content_type :json
    # doess the uuid for the home match the on in our mock database
...
# This is what will run the serce
